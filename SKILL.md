...
If a selector offers `Custom model`/`自定义模型`, it is only a branch, never a completed model choice. Immediately open its text input or ask the user in the next message: “请填写 `<role>` 的精确模型 ID（可包含 provider/routing 前缀）。” Do not continue to thinking depth, the next role, or initialization until the user supplies a non-empty exact identifier. If the host selector cannot collect text, do not present Custom as a terminal option: obtain the identifier in the conversation instead. Never persist UI labels such as `custom`, `custom model`, `自定义模型`, `other`, or `其他`; arbitrary concrete identifiers remain allowed, but are declared identities rather than proof of upstream routing.
